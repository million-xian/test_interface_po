...
def get_entrypoints(filename):
    ...
    # get the entry points and then the scripts names
...
def message_about_scripts_not_on_PATH(scripts):
    ...
    for parent_dir, scripts in warn_for.items():
        ...
        if len(scripts) == 1:
            start_text = "scripts {} is".format(scripts[0])
        else:
    ...
    last_line_fmt = (
        ...
        "to suppress this warning, use --no-warn-scripts-location."
    )
    if len(msg_lines) == 1:
...
def move_wheel_files(
    ...
    #   generated = files newly generated during the install (scripts wrappers)
    installed = {}  # type: Dict[str, str]
    changed = set()
    generated = []  # type: List[str]
    ...
    def is_entrypoint_wrapper(name):
        # EP, EP.exe and EP-scripts.py are scripts generated for
        # entry point EP by setuptools
        if name.lower().endswith('.exe'):
            matchname = name[:-4]
        elif name.lower().endswith('-scripts.py'):
            matchname = name[:-10]
    ...
    # Simplify the scripts and fix the fact that the default scripts swallows
    ...
    def _get_script_text(entry):
        if entry.suffix is None:
            raise InstallationError(
                "Invalid scripts entry point: %s for req: %s - A callable "
